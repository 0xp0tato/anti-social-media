...
describe('Test validity of email input', () => {
    ...
    it('Should return 442 if email is not provided', async () => {
        await request(app).post(APP_ROUTES.SIGNUP_ROUTE).send({ password }).expect(422);
    });

    it('Should return 442 if email is not valid', async () => {
        await request(app).post(APP_ROUTES.SIGNUP_ROUTE).send({ email: 'asdasdasd', password }).expect(422);
    });
    ...
    it('Should return 200 if email is valid', async () => {
        await request(app).post(APP_ROUTES.SIGNUP_ROUTE).send({ email: 'test@email.com', password }).expect(201);
    });
});
...
describe('Test validity of password input', () => {
    ...
    it('Should return 442 if no password is provided', async () => {
        await request(app).post(APP_ROUTES.SIGNUP_ROUTE).send({ email }).expect(422);
    });

    it('Should return 442 if password length is less than 8 characters', async () => {
        await request(app).post(APP_ROUTES.SIGNUP_ROUTE).send({ email, password: 'Pass.1' }).expect(422);
    });

    it('Should return 442 if password has no lowercase letter', async () => {
        await request(app).post(APP_ROUTES.SIGNUP_ROUTE).send({ email, password: 'PASSWORD.1' }).expect(422);
    });
    ...
    it('Should return 442 if password has no number', async () => {
        await request(app).post(APP_ROUTES.SIGNUP_ROUTE).send({ email, password: 'Password.' }).expect(422);
    });

    it('Should return 442 if password has no special character', async () => {
        await request(app).post(APP_ROUTES.SIGNUP_ROUTE).send({ email, password: 'Password1' }).expect(422);
    });

    it('Should return 200 if password is valid', async () => {
        await request(app).post(APP_ROUTES.SIGNUP_ROUTE).send({ email, password: 'Password.1' }).expect(201);
    });
});
...
describe('Test signup route method availability', () => {
    ...
    it('Should return 405 for GET request', async () => {
        await request(app).get(APP_ROUTES.SIGNUP_ROUTE).send({ email, password }).expect(405);
    });

    it('Should return 405 for PUT request', async () => {
        await request(app).put(APP_ROUTES.SIGNUP_ROUTE).send({ email, password }).expect(405);
    });

    it('Should return 405 for PATCH request', async () => {
        await request(app).patch(APP_ROUTES.SIGNUP_ROUTE).send({ email, password }).expect(405);
    });

    it('Should return 405 for DELETE request', async () => {
        await request(app).delete(APP_ROUTES.SIGNUP_ROUTE).send({ email, password }).expect(405);
    });

    it('Should return 200 for POST requests', async () => {
        await request(app).post(APP_ROUTES.SIGNUP_ROUTE).send({ email, password }).expect(201);
    });

    it('Should return 200 for OPTION request', async () => {
        await request(app).options(APP_ROUTES.SIGNUP_ROUTE).send({ email, password }).expect(200);
    });

    it('Should return available method options for OPTION request', async () => {
        const response = await request(app).options(APP_ROUTES.SIGNUP_ROUTE).send({ email, password }).expect(200);
